...
{
	err = set_val - read_val;				//uchyb
	err_sum = err_sum +	err;				//ca³kowanie

	if (err_sum > ERR_SUM_MAX) {			//ograniczenie ca³kowania
			err_sum = ERR_SUM_MAX;
	} else if (err_sum < -ERR_SUM_MAX) {
			err_sum = -ERR_SUM_MAX;
	}

	err_d = err_last - err;					//ró¿niczka

	u = kp * err + ki * err_sum + kd * err_d;

	err_last = err;							//aktualizacja ostatniego odczytu

...
volatile double set_value;				//zadana ilosc tykniec enkodera na 5ms
volatile double enc_value;				//zczytana --||--
volatile double set_value_obrot_min;	//zadana predkosc katowa wyrazona w obr/min

volatile double enc_value_diff[400];	//liczba tykniec enkodera przy starcie co 5ms

volatile uint16_t pulse_count1;			//zczytane dwa stany enkodera, odstep 5ms
...
volatile bool 	FLAG_END_OF_INSTR;
volatile bool 	FLAG_CORRECTION_OF_INSTR;

volatile uint8_t tab[BUFFER_SIZE];	//tablica - cala ramka po UART
volatile uint8_t tab_dma[INSTR_SIZE];//tablica jednoznakowa - komenda z UART przesylana przez DMA
volatile uint8_t tab_instrukcje[INSTR_SIZE];
volatile uint8_t Vx;
volatile uint8_t Vy;
////////////////////////////////
volatile uint8_t Vx_last;
volatile uint8_t Vy_last;
volatile bool zmiana_stanu;
///////////////////////////////

volatile uint16_t wypelnienie;
volatile uint16_t wypelnienie_x;
volatile uint16_t wypelnienie_y;
volatile int k;

...
void sterowanie_xy(volatile uint8_t buffer[]){
	if(FLAG_CORRECTION_OF_INSTR==true && FLAG_END_OF_INSTR==true){
		...
		if(Vx<0x21 || Vx>0x7E)		stop();															//wartosci wykraczajace poza zakres Vx
			...
			else if(Vx>0x50  && Vy>0x53) {				przod(wypelnienie_y*3/4);}					//PRZOD + PRAWO
			else if(Vx>0x50  && Vy<0x47) {				tyl(wypelnienie_y*3/4);}					//TYL + PRAWO
			else if(Vx<0x50  && Vy<0x47) {				tyl(wypelnienie_x/4 + wypelnienie_y*3/4);}	//TYL + LEWO
			else if(Vx<0x50  && Vy>0x53) {				przod(wypelnienie_x/4 + wypelnienie_y*3/4);}//PRZOD + LEWO
		}
	}
	else{
